...
$sql = "SELECT * FROM jos1n_orders WHERE 1 ORDER BY timestamp DESC";
...
if (mysqli_num_rows($result) > 0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
        ...
        if($status == "Booked"){
        	$button = "<button class='btn' id='pickup' value='" . $id . "'>Pickup</button><br /><br /><button class='btn' id='cancelbooking' value='" . $id . "'>Cancel Booking</button>";
        }
